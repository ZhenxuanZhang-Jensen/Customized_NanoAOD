...
import argparse
import json
with open('das_samples_UL16_data_resubmit.json', 'r') as f:
...
sed -i "s#MiniAODFileWithPath#${{3}}#g"  data2016_NANO.py
# cat for debug, as this will be printed out in stdout of condor log
cat data2016_NANO.py
#cat ${{3}}
echo "========================="
echo "==> cmsRun   data2016_NANO.py"
...
makeTarFile.make_tarfile(cmsswDirPath, CMSSWRel + "16_data.tgz")
...
print "copying the " + CMSSWRel + "16_data.tgz  file to eos path: " + storeDir + "\n"
os.system('cp ' + CMSSWRel + "16_data.tgz" + ' ' + storeDir + '/' + CMSSWRel + "16_data.tgz")
# # Step - 3: Get .sh and .jdl file; as produced below
CondorExecutable = "condor_submit_UL16_data"
with open(CondorExecutable + ".sh","w") as fout:
    fout.write(sh_file_template.format(     cmsswTarFileWithPath = storeDir + '/CMSSW_10_6_2516_data.tgz',
                                            NameOfTarFile = CMSSWRel + "16_data.tgz",
...
print "\tvoms-proxy-init --voms cms --valid 188:00";
