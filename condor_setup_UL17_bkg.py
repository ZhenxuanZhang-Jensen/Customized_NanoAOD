...
sed -i "s#MiniAODFileWithPath#${{3}}#g"  mc2017_NANO.py
# cat for debug, as this will be printed out in stdout of condor log
cat mc2017_NANO.py
#cat ${{3}}
echo "========================="
echo "==> cmsRun   mc2017_NANO.py"
...
makeTarFile.make_tarfile(cmsswDirPath, CMSSWRel + "_" + running_files_name + "_submit.tgz")
...
print "copying the " + CMSSWRel + "_" + running_files_name + "_submit.tgz  file to eos path: " + storeDir + "\n"
os.system('cp ' + CMSSWRel + "_" + running_files_name + "_submit.tgz" + ' ' + storeDir + '/' + CMSSWRel + "_" + running_files_name + "_submit.tgz")
# # Step - 3: Get .sh and .jdl file; as produced below
CondorExecutable = "condor_submit_" + "_" + running_files_name
with open(CondorExecutable + ".sh","w") as fout:
    fout.write(sh_file_template.format(     cmsswTarFileWithPath = storeDir + '/' + CMSSWRel + "_" + running_files_name + "_submit.tgz",
                                            NameOfTarFile = CMSSWRel + "_" + running_files_name + "_submit.tgz",
                                            pathWhereCodePlaced = "CMSSW_10_6_25/src"
                                            ))
for i in range(len(sample_card.keys())):
  command = "mkdir " +"log/" + sample_card.keys()[i]
