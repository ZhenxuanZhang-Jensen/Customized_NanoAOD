...
import argparse
import json
with open('das_samples_NMSSM_XToYHTo2G2WTo4Q.json', 'r') as f:
...
sed -i "s#MiniAODFileWithPath#${{3}}#g"  mc2018_NANO.py
# cat for debug, as this will be printed out in stdout of condor log
cat mc2018_NANO.py
#cat ${{3}}
echo "========================="
echo "==> cmsRun   mc2018_NANO.py"
...
makeTarFile.make_tarfile(cmsswDirPath, CMSSWRel + "_18_submit.tgz")
...
print "copying the " + CMSSWRel + "_18_submit.tgz  file to eos path: " + storeDir + "\n"
os.system('cp ' + CMSSWRel + "_18_submit.tgz" + ' ' + storeDir + '/' + CMSSWRel + "_18_submit.tgz")
# # Step - 3: Get .sh and .jdl file; as produced below
CondorExecutable = "condor_submit_UL18_all_signal"
with open(CondorExecutable + ".sh","w") as fout:
    fout.write(sh_file_template.format(     cmsswTarFileWithPath = storeDir + '/' + CMSSWRel + "_18_submit.tgz",
                                            NameOfTarFile = CMSSWRel + "_18_submit.tgz",
